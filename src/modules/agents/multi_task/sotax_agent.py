...
class SotaXAgent(nn.Module):
    ...
    def __init__(self, task2input_shape_info, task2decomposer, task2n_agents, decomposer, args):
        ...
        change:cyd
        '''
        for t in task2decomposer:
            taskDecomp = task2decomposer[t]
            obs_own_dim = max(obs_own_dim, taskDecomp.own_obs_dim)
            obs_en_dim = max(obs_en_dim, taskDecomp.obs_nf_en)
            obs_al_dim = max(obs_al_dim, taskDecomp.obs_nf_al) 
            
        pad = 0
        for t in task2decomposer:
            taskDecomp = task2decomposer[t]
            pad = max(pad, obs_own_dim-taskDecomp.own_obs_dim)
            pad = max(pad, obs_en_dim-taskDecomp.obs_nf_en)
            pad = max(pad, obs_al_dim-taskDecomp.obs_nf_al)
        decomposer.pad = pad
    ...
    def forward(self, inputs, hidden_state, task_repre, task):
        ...
        change:cyd
        '''
        own_obs = F.pad(own_obs, (0, self.own_value.in_features - own_obs.size(1)), "constant", 0)
        ...
        change:cyd
        '''
        ally_feats = F.pad(ally_feats, (0, self.ally_key.in_features - ally_feats.size(-1)), "constant", 0)
        ...
        change:cyd
        '''
        enemy_feats = F.pad(enemy_feats, (0, self.enemy_key.in_features - enemy_feats.size(-1)), "constant", 0)
