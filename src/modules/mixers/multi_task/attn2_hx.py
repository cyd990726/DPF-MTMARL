...
class QMixer(nn.Module):
    def __init__(self, surrogate_decomposer, main_args):
        ...
        if self.state_last_action:
            ...
            change:cyd
            '''
            # if(surrogate_decomposer.map_type == "marines"):
            self.ally_encoder = nn.Linear(state_nf_al + self.n_actions_no_attack + 1 + surrogate_decomposer.pad, self.entity_embed_dim)
            self.enemy_encoder = nn.Linear(state_nf_en + surrogate_decomposer.pad, self.entity_embed_dim)
            ...
            '''
            # if(surrogate_decomposer.map_type == "marines"):
            self.ally_encoder = nn.Linear(state_nf_al + surrogate_decomposer.pad, self.entity_embed_dim)
            self.enemy_encoder = nn.Linear(state_nf_en + surrogate_decomposer.pad, self.entity_embed_dim)
    ...
    def forward(self, agent_qs, states, task_repre, task_decomposer):
        ...
        change:cyd
        '''
        ally_states = F.pad(ally_states, (0, self.ally_encoder.in_features - ally_states.size(-1)), "constant", 0)
        enemy_states = F.pad(enemy_states, (0, self.enemy_encoder.in_features - enemy_states.size(-1)), "constant", 0)
